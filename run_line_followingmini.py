...
COEFF_ERR = 90/320
def go_side_forward(robot, current_speed,bonus):
    # if bonus_speed != 0:
    #     current_speed=current_speed*1/(abs(bonus_speed)*2)
    # if(abs(bonus)>0.8):
    #     bonus_speed=bonus*2
    #     malus_speed=bonus*1.2
    if(abs(bonus)>0.5):
        bonus_speed=bonus*1.5
        malus_speed=bonus*0.6
    
    else:
        bonus_speed=bonus*0.5
        malus_speed=bonus*0.5
    if bonus_speed>0:  
